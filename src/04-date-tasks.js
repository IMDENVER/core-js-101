...
function angleBetweenClockHands(date) {
  const time = new Date(date).toISOString().slice(11, -8);
  let hour = time.slice(0, -3);
  const minute = time.slice(3, time.length);
  if (hour > 12) {
    hour -= 12;
  }
  const hourAngle = 30 * hour + 0.5 * minute;
  const minAngle = 6 * minute;
  let degree;
  if (hourAngle > minAngle) {
    degree = hourAngle - minAngle;
  } else {
    degree = minAngle - hourAngle;
  }
  if (degree > 180) {
    degree -= 180;
  }
